...
TENANT_ID=`curl -s -X POST https://identity.api.rackspacecloud.com/v2.0/tokens -d "{\"auth\":{\"passwordCredentials\":{\"username\":\"$USERNAME\",\"password\":\"$PASSWORD\" }},\"tenantId\": \"$OS_TENANT_ID\"}" -H "Content-type: application/json" | python -m json.tool | grep tenantId | grep Mosso  | cut -d ":" -f2 | sort | uniq | sed -e s/\"//g  | sed -e "s/ //g"`
